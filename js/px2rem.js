function px2rem(winw, winh) {
    var ua = window.navigator.userAgent;
    var IS_PHONE = false;
    if (/Android\s+([\d.]+)/.test(ua) || /(?:iPad|iPhone).*OS\s([\d_]+)/.test(ua)) {
        IS_PHONE = true;
    }
    if (IS_PHONE) {
        var docEle = window.document.documentElement,
            boundingWidth = winw || docEle.clientWidth,
            boundingHeight = winh || docEle.clientHeight;
        docEle.className = "l-phone";
        if (boundingWidth <= 320 && boundingHeight <= 430) {
            docEle.className += " iphone4";
        }
        !function(J, I) {
            function H() {
                var a = Math.min(boundingWidth, boundingHeight);
                a / B > 540 && (a = 540 * B);
                var d = a / 10;
                E.style.fontSize = d + "px",
                z.rem = J.rem = d;
            }
            var G, F = J.document, E = F.documentElement, D = F.querySelector('meta[name="viewport"]'), C = F.querySelector('meta[name="flexible"]'), B = 0, A = 0, z = I.flexible || (I.flexible = {});
            if (D) {
            ...
            !D) {
                ...
                D.setAttribute("content", "initial-scale=" + A + ", maximum-scale=" + A + ", minimum-scale=" + A + ", user-scalable=no"),
                E.firstElementChild) {
